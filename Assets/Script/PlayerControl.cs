...
public class PlayerCtrl :MonoBehaviour
{
    ...
    public float runningModifier = 7.0f/5.0f;
    ...
    void Update ()
    {
        ...
        if (this.totalWeight > gameManager.maxWeight) {
            moveSpeed = startMoveSpeed/(gameManager.waterLevel*0.01f*2f + 1);
            tired = true;
        } else {
            moveSpeed = startMoveSpeed/(gameManager.waterLevel*0.01f + 1);
            tired = false;
        }
    ...
    void FixedUpdate ()
    {
        if (running && Stamina>0 && !tired && canMove) {
            rb.linearVelocity = new Vector2(speedX * moveSpeed * runningModifier, speedY * moveSpeed * runningModifier);
